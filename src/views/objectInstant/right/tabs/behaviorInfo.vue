<template>
	<div class='behavior-info' v-if="objectDetail.id">
		<div class="top" :class="{notEdit:!ifEdit}">
			<div class="accordion-top">
				<el-collapse v-model="accordionName" accordion class="accordion" v-if="topData && topData.length>0">
					<el-collapse-item v-for="(n,i) in topData" :key="i" :title="getName(n)" :name='i' v-drag="{data:n,id:'behavior'}" v-if="n">
						<el-form label-position="right" label-width="75px" :model="n">
		...
		<div class="down" v-if="ifEdit">
			...
			<div class="drag-in" v-drop.stop="dropBehavior" @click="openTab('behaviorList')">
				点击添加行为
				<!--+拖拽到此处添加-->
			</div>
...
<script>
	...
	import behaviorService from "@/psde/behaviorService";
	...
	export default {
		...
		mounted() {
			...
			// let context = IdEdit.osmContent;
			// console.log(context)
			// setTimeout(() => {
			//   let id = 'w5120213360640';
		...
		methods: {
			...
			//         });
			//       });
			//       this.accordionData = Object.assign([], arr);
			//       this.oldData = Object.assign([], arr);
			//     });
			// },
			dropBehavior(data) {
				//拖拽结束
				if(data.data.id == "behavior") {
