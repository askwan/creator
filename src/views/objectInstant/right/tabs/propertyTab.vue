				...
				<div class="object-attributes-detail">
					...
					<div class="object-attributes-value">
						<div class="object-attributes-list" v-for="(item,index) in objectAttrLists" :key="index">
							...
							<input v-if="getIntypeVal(item.fid)=='Text' || getIntypeVal(item.fid)=='Textarea'" type="text" v-model="item.value" :disabled="!ifEdit" @blur="lastValue(item,index)" placeholder="请输入内容" />
							<input v-else-if="getIntypeVal(item.fid)=='Number'" type="number" :placeholder="'范围：'+getMinVal(item.fid)+' ~ '+getMaxVal(item.fid)" :min="getMinVal(item.fid)" :max="getMaxVal(item.fid)" v-model="item.value" :disabled="!ifEdit" @blur="lastValue(item,index)" />
							...
							<!--<el-select v-else-if="getIntypeVal(item.fid)=='Checkbox'" v-model="item.value" @change="checkboxValue(item,index)" multiple filterable collapse-tags allow-create default-first-option placeholder="请选择,可多选,可输入">-->
							...
							<input v-else type="text" v-model="item.value" :disabled="!ifEdit" @blur="lastValue(item,index)" placeholder="请输入内容" />
							<i class="iconfont icon-shanchu11" v-if="getDeleteShow(item,index)" @click="deleteAttr(item,index)"></i>
						</div>
						<div class="add-diy" @click="addProperty" v-if="ifEdit">
...
<script>
	...
	export default {
		...
		props: ["ifEdit", "objectDetail", "diagrams"],
		components: {
			//otypeList,
		},
		activated() {},
