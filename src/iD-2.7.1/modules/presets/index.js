...
export function presetIndex() {
  ...
  all.init = function (fieldsArr, otypesArr, formDict) {
    fieldsArr = fieldsArr || [];
    otypesArr = otypesArr || [];
    formDict = formDict || []
    var d = data.presets;
    ...
    //   _fields[item.name.toString()] = presetField(item.caption, {
    //     "key": item.name,
    //     "type": getType(item.uitype),
    //     "label": item.caption,
    //     "universal": true,
    //     "placeholder": item.des,
    //     "strings": {
    //       "options": getComboOptions(item.domain)
    //     }
    //   });
    // })
    ...
    // var fCollection, geoCollection;
    // var netOtype = presetCollection([]);
    // otypesArr.forEach(item => {
    //   item.fields = item.fields || {};
    //   item.fields.fields = item.fields.fields || [];

    //   fCollection = item.fields.fields.map(el => el.name.toString());
    //   // fCollection = ["name"].concat(fCollection);
    //   item.formStyles = item.formStyles || {};
    //   item.formStyles.styles = item.formStyles.styles || [];
    //   geoCollection = item.formStyles.styles.map(el => getGerm(formDict, el.type));
    //   all.collection.push(presetPreset(item.id.toString(), {
    //     "fields": fCollection,
    ...
    //     "name": item.name
    //   }, _fields));
    //   netOtype.collection.push(presetPreset(item.id.toString(), {
    //     "fields": fCollection,
    //     "geometry": geoCollection,
    //     "tags": {
    //       [item.name]: "*"
    ...
    if (d.defaults) {
      ...
      // _defaults = {
      //   area: netOtype.matchGeometry("point"),
      //   line: netOtype.matchGeometry("line"),
      //   point: netOtype.matchGeometry("area"),
      //   vertex: netOtype.matchGeometry("point"),
      //   relation: netOtype.matchGeometry("relation")
      // };
    }
...
function getComboOptions(str) {
  str = str || "";
  var option;
  try {
    option = str.split("[")[1].split("]")[0].split(",");
  } catch (error) {
    option = [];
  }
  var options = {};
  option.forEach(item => options[item] = item)
  return options
}
